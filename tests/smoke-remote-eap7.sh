...
function test_jbossas() {
    service eap7-standalone start && firefox http://localhost:9990 && firefox http://localhost:8080/mass-bugzilla-modifier
    service eap7-standalone stop
}

function test_jbossas_domain() {
    service eap7-domain start && firefox http://localhost:9990
